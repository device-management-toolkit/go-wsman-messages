...
Minimum supported Go version: **1.25.0** (`go.mod`). CI, integration, and release workflows use Go **1.27**. Keep the module minimum separate from CI toolchain updates; raise it only when required by source code or dependencies, and explain the impact on consumers. Write platform-agnostic code.
