...
type (
	SettingDataRequest struct {
		...
		NonSecureConnectionsSupported *bool    `xml:"h:NonSecureConnectionsSupported,omitempty"` // Indicates the removal of support for the non-TLS WS-MAN ports for the remote interface. This is read-only in firmware, so omit it from Put requests unless a caller explicitly opts in.
	}
)
