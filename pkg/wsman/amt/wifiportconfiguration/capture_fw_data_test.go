...
package wifiportconfiguration_test
...
func TestCaptureFirmwareXMLWiFiPortConfigurationService(t *testing.T) {
	...
	service := wifiportconfiguration.NewWiFiPortConfigurationServiceWithClient(wsmanMessageCreator, wsmanClient)
	defer wsmanClient.CloseConnection()

	enumerateResponse, err := service.Enumerate()
	if err != nil {
		t.Fatalf("enumerate error: %v", err)
	}

	pullResponse, err := service.Pull(enumerateResponse.Body.EnumerateResponse.EnumerationContext)
	if err != nil {
		t.Fatalf("pull error: %v", err)
	}

	if len(pullResponse.Body.PullResponse.WiFiPortConfigurationItems) == 0 {
		t.Fatal("pull returned no AMT_WiFiPortConfigurationService instances")
	}

	getResponse, err := service.GetByName(pullResponse.Body.PullResponse.WiFiPortConfigurationItems[0].Name)
	if err != nil {
	...
	defaultOutputDir := filepath.Join(
		...
		"..", "..", "..", "wsmantesting", "responses", "amt", "wifiportconfiguration",
	)
