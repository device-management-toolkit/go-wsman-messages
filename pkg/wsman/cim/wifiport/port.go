...
// Package wifiport implements CIM_WiFiPort.
//
// CIM_WiFiPort: A class derived from NetworkPort that provides the logical representation of
// wireless local area network communications hardware that conforms to the IEEE 802.11 series of specifications.
package wifiport
...
// NewWiFiPortWithClient returns a new instance of the WiFiPort struct.
...
func (p Port) RequestStateChange(requestedState int) (response Response, err error) {
	response = Response{
		Message: &client.Message{
			XMLInput: p.Base.RequestStateChange(methods.GenerateAction(CIMWiFiPort, "RequestStateChange"), requestedState),
		},
	}
